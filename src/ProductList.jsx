...
function ProductList({ onHomeClick }) {
    ...
    const [showPlants, setShowPlants] = React.useState(false); // State to control the visibility of the About Us page
                ...
                {plantsArray.map((category, index) => (
                    <div key={index}> {/* Unique key for each category div */}
                        ...
                        <div className="product-card" key={plantIndex}> {/* Unique key for each plant card */}
                            ...
                            <button
                                className={`product-button ${isPlantAdded(plant) ? 'added-to-cart' : ''}`}
                                disabled={isPlantAdded(plant)}
                                onClick={() => handleAddToCart(plant)} // Handle adding plant to cart
                            >
                                {isPlantAdded(plant) ? 'Added to Cart' : 'Add to Cart'}
                            </button>
                        </div>
                        ))}
                        </div>
